 ...
 * @param string $str_method (default: 'post')
 * @return void
 */
function fn_https($str_url, $arr_data, $file_data=array(), $str_method = 'post') {
	$files_type = array(
		'.jpg',
		'.png',
		'.gif',
	...
	if(is_array($file_data) && !empty($file_data)){
		...
		foreach($file_data as $key=>$value){
			if(!in_array(strrchr($value,'.'),$files_type)){
				return 	false;
			}
			
			if(class_exists('finfo')){
				$fi = new finfo(FILEINFO_MIME_TYPE); 
				$mime_type = $fi->file($value); 
			}else{
				$mime_type = mime_content_type($value);
			}
			
			$file_post[$key] = curl_file_create($value,$mime_type,$key);
		}
		
	}
	
	if(is_array($file_post) && !empty($file_post)){
		$postdata = array_merge($_str_data,$file_post);	
	}else{
		$postdata = $_str_data;	
	}	
    ...
    if ($str_method == 'post') {
        curl_setopt($_obj_http, CURLOPT_POST, true);
        curl_setopt($_obj_http, CURLOPT_POSTFIELDS, $postdata);
        curl_setopt($_obj_http, CURLOPT_URL, $str_url);
    } else {
        if (stristr($str_url, '?')) {
            $_str_conn = '&';
        ...
        curl_setopt($_obj_http, CURLOPT_URL, $str_url . $_str_conn . $postdata);
    }
...
//判断是否base64加密
function fn_isbase64($str) {
    //这里多了个纯字母和纯数字的正则判断
    if (@preg_match('/^[0-9]*$/', $str) || @preg_match('/^[a-zA-Z]*$/', $str)) {
        return false;
    }
    elseif(fn_isutf8(base64_decode($str)) && base64_decode($str) != '') {
        return true;
    }
    return false;
}
